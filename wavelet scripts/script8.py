...
ax.set_title("Todas posicoes de buracos detectados usando TWH cascata $D_{" + str(wav_levels)+"}$ e $\lambda_1$ - Ida")
...
for index, file_ in enumerate(files_in_dir):
    ...
    for i, v in enumerate(complementar):
        if(v[0] != 0) and file_.startswith('ida'):
...
plt.gcf().text(0.01, 0.90, "21 arquivos", fontsize=12)
#plt.savefig(path + "cascata-todos-hits-ida-wav"+str(wav_levels)+".png",dpi=500)
plt.show()
plt.close()
